...
## quality elasticity by subgroup (for the heterogeneity figure)
sub_q <- function(d, label) { v <- rob(lm(ln_uv ~ ln_w + ln_ae + wave + region + item, d))
  tibble(subgroup = label, eps_quality = v[1], se = v[2]) }
subgroup_elast <- bind_rows(
  sub_q(filter(est_p, rural == 1), "Rural"), sub_q(filter(est_p, urban == 1), "Urban"),
  sub_q(filter(est_p, livestock == 1), "Owns livestock"), sub_q(filter(est_p, livestock == 0), "No livestock"))
...
het_test <- function(inter) { m <- lm(as.formula(paste0("ln_uv ~ ln_w + ln_w:", inter,
  ...
  c(diff = ct[r, 1], p = ct[r, 4]) }
het_urban <- het_test("urban"); het_liv <- het_test("livestock")
## quality elasticity significant in all quartiles? (for the income question)
...
n_q_sig <- sum(pt(ctq[sr, 1]/ctq[sr, 2], df.residual(m_byq), lower.tail = FALSE) < 0.05)


## ============================================================================
## OUTPUTS IN OUTLINE ORDER (1-15)
## ============================================================================
theme_set(theme_minimal(base_size = 12))

## --- 1. Sample descriptive statistics (full / rural-urban / livestock) ------
...
tab_01 <- bind_rows(samp(household, "Full sample"),
  ...
  samp(filter(household, livestock == 1), "Owns livestock"), samp(filter(household, livestock == 0), "No livestock"))
write.csv(tab_01, file.path(out_tab, "01_sample_descriptives.csv"), row.names = FALSE)
## consumption and expenditure by product
write.csv(item_summary, file.path(out_tab, "01b_consumption_expenditure_by_product.csv"), row.names = FALSE)

## --- 2. Consumption is stable across waves (levels by product x wave) --------
tab_02 <- cons_wave %>% pivot_wider(names_from = wave, values_from = qty_pae)
write.csv(tab_02, file.path(out_tab, "02_consumption_by_wave.csv"), row.names = FALSE)

## --- 3. Share of households consuming each item -----------------------------
g <- ggplot(item_summary, aes(reorder(Item, `% consuming`), `% consuming`, fill = Item)) +
  ...
  labs(title = "3. Share of households consuming each item", x = NULL, y = "% (past 7 days)")
ggsave(file.path(out_fig, "03_share_consumers.png"), g, width = 8, height = 5, dpi = 150)

## --- 4. Product-level descriptive statistics (expenditure per AE) -----------
g <- ggplot(item_summary, aes(reorder(Item, `Expenditure/AE`), `Expenditure/AE`, fill = Item)) +
  geom_col(show.legend = FALSE) + scale_fill_manual(values = pal) + coord_flip() +
  labs(title = "4. Product-level descriptive statistics", subtitle = "Expenditure per adult equivalent (consumers)",
       x = NULL, y = "Expenditure per AE (TSH, 7 days)")
ggsave(file.path(out_fig, "04_product_descriptives.png"), g, width = 8, height = 5, dpi = 150)

## --- 5. Consumption trends across survey waves ------------------------------
g <- cons_wave %>% group_by(Item) %>% arrange(wave) %>%
  ...
  labs(title = "5. Consumption trends across survey waves", subtitle = "Quantity per AE, index Wave Y3 = 100",
       x = "Survey wave", y = "Quantity per AE (index)", colour = NULL)
ggsave(file.path(out_fig, "05_consumption_trends_waves.png"), g, width = 9, height = 5, dpi = 150)

## --- 6a. Household composition urban/rural by wave --------------------------
g <- household %>% group_by(wave) %>%
  ...
  labs(title = "6a. Household composition by wave", x = NULL, y = "%", fill = NULL)
ggsave(file.path(out_fig, "06a_hh_composition_by_wave.png"), g, width = 8, height = 5, dpi = 150)

## --- 6b. Urban vs rural consumption (expenditure per AE) --------------------
g <- group_tables %>% filter(area %in% c("rural", "urban"), n_obs >= min_cell) %>%
  ...
  labs(title = "6b. Urban vs rural consumption", subtitle = "Expenditure per adult equivalent (TSH)",
       x = NULL, y = "Expenditure per AE (TSH, 7 days)", fill = NULL) +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))
ggsave(file.path(out_fig, "06b_urban_rural_consumption.png"), g, width = 9, height = 5, dpi = 150)

## --- 7. Sources of food consumption -----------------------------------------
g <- source_decomp %>% pivot_longer(c(Purchased, `Own production`, Gifts), names_to = "Source", values_to = "v") %>%
  ...
  labs(title = "7. Sources of food consumption", subtitle = "Market purchases / own production / gifts",
       x = NULL, y = "Share of consumed quantity", fill = NULL) +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))
ggsave(file.path(out_fig, "07_sources.png"), g, width = 9, height = 5, dpi = 150)

## --- 8. Consumption per AE by income quartile (six products) ----------------
g <- gt_all %>% filter(n_obs >= min_cell) %>% idx_q("qty_pae") %>%
  ...
  scale_colour_manual(values = pal) +
  labs(title = "8. Consumption per adult equivalent by income quartile", subtitle = "Pooled; index, Q1 = 100",
       x = "Income quartile (poor -> rich)", y = "Quantity per AE (index)", colour = NULL)
ggsave(file.path(out_fig, "08_consumption_by_quartile.png"), g, width = 9, height = 5, dpi = 150)

## --- 9. Expenditure per AE by income quartile (six products) ----------------
g <- gt_all %>% filter(n_obs >= min_cell) %>% idx_q("exp_pae") %>%
  ggplot(aes(group, Index, colour = item_label, group = item_label)) + geom_line(linewidth = 1) + geom_point() +
  scale_colour_manual(values = pal) +
  labs(title = "9. Expenditure per adult equivalent by income quartile", subtitle = "Pooled; index, Q1 = 100",
       x = "Income quartile (poor -> rich)", y = "Expenditure per AE (index)", colour = NULL)
ggsave(file.path(out_fig, "09_expenditure_by_quartile.png"), g, width = 9, height = 5, dpi = 150)

## --- 10. Consumption vs expenditure vs unit value (six panels) --------------
...
g <- ggplot(panel, aes(group, Index, colour = Measure, group = Measure)) + geom_line(linewidth = 1) + geom_point() +
  ...
  labs(title = "10. Consumption vs expenditure vs unit value across income quartiles",
       subtitle = "Index, Q1 = 100; expenditure above quantity = quality", x = "Income quartile (poor -> rich)",
       y = "Index (Q1 = 100)", colour = NULL)
ggsave(file.path(out_fig, "10_consumption_vs_expenditure_panels.png"), g, width = 11, height = 6, dpi = 150)

## --- 11. Composition of the animal-food basket by income --------------------
g <- ggplot(budget_comp, aes(quartile, share, fill = item_label)) + geom_col() + scale_fill_manual(values = pal) +
  labs(title = "11. Composition of the animal-food basket by income",
...
ggsave(file.path(out_fig, "11_basket_composition.png"), g, width = 9, height = 5, dpi = 150)

## --- 12. Inequality ratios Q4/Q1 (table) ------------------------------------
write.csv(ratios, file.path(out_tab, "12_inequality_ratios_Q4_Q1.csv"), row.names = FALSE)
print(as.data.frame(ratios), digits = 3)

## --- 13. Quantity vs expenditure elasticities (table) -----------------------
write.csv(elasticities, file.path(out_tab, "13_elasticities.csv"), row.names = FALSE)
print(as.data.frame(elasticities), digits = 3)

## --- 14. Measuring the quality effect (the wedge) ---------------------------
g <- elasticities %>% filter(Item != "ALL ITEMS") %>%
  ...
  labs(title = "14. The quality effect: expenditure elasticity = quantity + quality",
       subtitle = "The quality part (top) is the quality-upgrading wedge",
       x = NULL, y = "Elasticity w.r.t. real expenditure per AE", fill = NULL) +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))
ggsave(file.path(out_fig, "14_quality_effect.png"), g, width = 9, height = 5, dpi = 150)

## --- 15. Heterogeneity in quality upgrading (hypothesis test) ---------------
sub <- sprintf("Urban vs rural: diff %+.3f (p=%.2f).  Livestock: diff %+.3f (p=%.2f).  Significant in %d/4 quartiles.",
               het_urban["diff"], het_urban["p"], het_liv["diff"], het_liv["p"], n_q_sig)
g <- subgroup_elast %>% mutate(lo = eps_quality - 1.96*se, hi = eps_quality + 1.96*se,
  ...
  labs(title = "15. Heterogeneity in quality upgrading (95% CI)", subtitle = sub,
       x = "Quality elasticity", y = NULL)
ggsave(file.path(out_fig, "15_heterogeneity_test.png"), g, width = 9, height = 5, dpi = 150)

cat("\nDone. Tables 01-13 and figures 03-15 in", normalizePath(out_tab), "and", normalizePath(out_fig), "\n")
